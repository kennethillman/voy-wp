...
<div class="b-repeater-list <?php echo esc_attr($className); ?> <?php echo esc_attr($class_margins_options_); ?> <?php echo esc_attr($class_divider_options); ?>">
    <div class="gc">
        <div class="g-12">
            ...
            <?php if(!empty($header) && $header!=''): ?>
                <h3 class="special-header -border-bottom"><?php echo $header; ?></h3>
            <?php endif; ?>

            <ul>
                <?php
                    if(!empty($list_repeater)):
                        foreach ($list_repeater as $repeater):
                ?>
                    <li>
                        <a href="<?php echo ($repeater['link']['url']!='')?$repeater['link']['url']:'#'; ?>">
                            <div class="text">
            ...
            <?php
                if(!empty($bottom_link) && $header!=''):
            ?>
                <a class="btn -black -icon -square -block -text-left"
                   href="<?php echo $bottom_link['url']; ?>"><?php echo $bottom_link['title']; ?>
